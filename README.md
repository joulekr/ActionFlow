...
### 🔎 하이브리드 통합 검색
- 기본 모드 검색창 하나로 앱·계산기뿐 아니라 스니펫·클립보드·즐겨찾기·파일까지 한 번에 검색
- exact match·frecency·종류별 가중치 기준으로 자동 정렬
- 특정 소스만 보고 싶을 땐 모드 탭 클릭이나 `/clip`, `/snippet` 같은 슬래시 키워드로 해당 탭으로 전환 가능

### 📋 클립보드 히스토리
- 텍스트 복사 시 자동 저장, 최대 200개 · SQLite 영구 저장
- 검색, 고정(Pin), 수정, 삭제 지원
- **출처 앱 표시** — `chrome · 2분 전` 형식으로 어느 앱에서 복사했는지 표시
- **서식 없이 붙여넣기** — 우클릭 또는 `Ctrl+Enter` → CF_UNICODETEXT만 설정, RTF/HTML 서식 제거 후 이전 창에 붙여넣기
- **이미지 OCR 텍스트 추출** — Windows 내장 OCR 엔진으로 클립보드 이미지에서 텍스트 인식, 결과를 바로 클립보드에 복사 가능
...
- `Ctrl+P`로 **미리보기** — 결과 리스트 옆에 나란히 표시되는 2-컬럼 레이아웃, 텍스트 파일 앞 10줄(JSON·Markdown·코드 신택스 하이라이팅 포함), 이미지는 미리보기 영역에 꽉 차게 표시
- 우클릭 → 폴더 열기 / 경로 복사 / 속성 / 관리자 권한으로 실행 / 즐겨찾기 추가
...
- [x] 파일 미리보기 기능 (v0.1.9)
- [x] 리마인더 기능 (v0.1.9)
- [x] 클립보드 출처 앱 표시 (v0.2.0)
- [x] 날짜,시간 계산 기능 (v0.2.0)
- [x] 탭 순서 변경 설정 (v0.2.1)
- [x] 이모지 카테고리 분류 (v0.2.2)
- [x] 투명도 조정 기능 (v0.2.2)
- [X] 클립보드 매니저에 이미지 지원 (V0.2.3)
...
- [x] 클립보드 서식 없이 붙여넣기 단축키 추가 (v0.2.6)
- [x] 앱 중복 실행 방지 (기존 창 포커스) (v0.2.6)
